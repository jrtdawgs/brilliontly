...
export function BrilliontlyLogo({ size = 40, className = '' }: { size?: number; className?: string }) {
  const id = useId();
  const gradId = `bGrad-${id}`;
  const gradBgId = `bGradBg-${id}`;
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id={gradBgId} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#4ade80" stopOpacity="0.15" />
          <stop offset="100%" stopColor="#22d3ee" stopOpacity="0.15" />
        </linearGradient>
      </defs>
      {/* Rounded square background */}
      <rect x="2" y="2" width="44" height="44" rx="12" fill={`url(#${gradBgId})`} stroke={`url(#${gradId})`} strokeWidth="2" />
      {/* Stylized B letter */}
      <path
